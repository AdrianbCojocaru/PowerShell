...
$global:TssVerDateDND = "2022.10.14.1"
...
Function CollectDND_WULogsLog
{
    ...
    if ([int]$_major -ge 10)  { $_WIN10_OR_LATER = 1}
	...
	$SourceDestinationPaths = @(
		...
		@("$Env:windir.old\Windows\windowsupdate.log", "$($_prefix)WindowsUpdate.old.log"),
		@("$Env:windir.old\Windows\SoftwareDistribution\ReportingEvents.log", "$($_prefix)ReportingEvents.old.log"),
		@("$_OLDLOCALAPPDATA\microsoft\windows\windowsupdate.log", "$($_prefix)WindowsUpdatePerUser.old.log"),
    ...
    # CBS & PNP logs
	$Commands = @(
		"robocopy.exe `"$Env:windir\logs\cbs`" 	$_tempdir\CBS *.log /W:1 /R:1 /NP /LOG+:$_robocopy_log | Out-Null"
		"robocopy.exe `"$Env:windir\logs\cbs`" 	$_tempdir\CBS *.cab /W:1 /R:1 /NP /LOG+:$_robocopy_log | Out-Null"
		"robocopy.exe `"$Env:windir\logs\dpx`" 	$_tempdir\CBS *.log /W:1 /R:1 /NP /LOG+:$_robocopy_log | Out-Null"
		"robocopy.exe `"$Env:windir\inf`" 		$_tempdir\CBS *.log /W:1 /R:1 /NP /LOG+:$_robocopy_log | Out-Null"
		"robocopy.exe `"$Env:windir\WinSxS`" 	$_tempdir\CBS poqexec.log /W:1 /R:1 /NP /LOG+:$_robocopy_log | Out-Null"
		"robocopy.exe `"$Env:windir\WinSxS`" 	$_tempdir\CBS pending.xml /W:1 /R:1 /NP /LOG+:$_robocopy_log | Out-Null"
		"robocopy.exe `"$Env:windir\servicing\sessions`" $_tempdir\CBS sessions.xml /W:1 /R:1 /NP /LOG+:$_robocopy_log | Out-Null"
	)
    ...
    # -------------------------------------------------------------
    # Windows Store logs.
    cmd /r Copy "$Env:temp\winstore.log" "$($_prefix)winstore-Broker.log" /y >$null 2>&1
    ...
    if (test-path -path $_WUETLPATH)
    {
        ...
        robocopy $_WUETLPATH $_tempdir\WU *.etl /W:1 /R:1 /NP /LOG+:$_robocopy_log >$null
	...
	{
		robocopy $_SIHETLPATH $_tempdir\SIH *.etl /W:1 /R:1 /NP /LOG+:$_robocopy_log >$null
	}
    ...
    robocopy "$Env:windir\logs\dism" $_tempdir\DISM * /W:1 /R:1 /NP /LOG+:$_robocopy_log >$null
    dism /online /get-packages /format:table > $_prefix'DISM_GetPackages.txt'
    dism /online /get-features /format:table > $_prefix'DISM_GetFeatures.txt'
    ...
    { 
        ...
        {
            FwCreateFolder $_tempdir\BVT 
            robocopy $bvtPath  $_tempdir\BVT *.log /W:1 /R:1 /NP /LOG+:$_robocopy_log >$null
        }
    }
    Write-Output "-------------------------------------------"
    ...
    $_evtx = $false
    $_format = "/TXT"
    Get-DNDEventLogs $_prefix $_tempdir $_robocopy_log $_errorfile $_line $_flush_logs $_event_logs $_evtx $_format
    ...
    { 
        ...
        {
            Copy-Item "$Env:windir\System32\winevt\Logs\$($_event_log).evtx" "$($_prefix)evt_$(($_event_log).replace('Microsoft-Windows-','')).evtx"
        } 
    }
    ...
    {
        try { Import-Module appx;get-appxpackage -allusers | Out-File -FilePath $_prefix'GetAppxPackage.log' }
        catch { LogException "Get-Appxpackage failed" $_ }
    }
    if ($_WINBLUE_OR_LATER -eq $true)
    {
        try { Get-Appxpackage -packagetype bundle | Out-File -FilePath $_prefix'GetAppxPackageBundle.log' }
        catch { LogException "Get-Appxpackage failed" $_ }
    }
...
Function CollectDND_SETUPReportLog
{
    ...
    } catch { LogException ("[$LogPrefixComputerSystem] Failed to query Win32_ComputerSystem class.") }
...
function Get-DNDWindowsUpdateInfo
{
    ...
    } catch { LogException ("[$LogPrefixWU] Failed to retrieve installed Updates from Win32_QuickFixEngineering class.") }
    ...
    if ((Get-Service wuauserv).StartType -ne [System.ServiceProcess.ServiceStartMode]::Disabled)
    {
        try
        {
            ...
            $Searcher = $Session.CreateUpdateSearcher()
            $historyCount = $Searcher.GetTotalHistoryCount()
            if($historyCount -gt 0) { $null = $Searcher.QueryHistory(0, $historyCount) | Select-Object -property *,@{Name = 'UpdateIdentityGUID';Expression  = {$_.UpdateIdentity.UpdateID}},@{Name = 'CategoryName';Expression = {$_.categories |Select-Object -First 1 -ExpandProperty Name}} -ExcludeProperty UpdateIdentity,Categories,UninstallationSteps,UninstallationNotes | Out-File -FilePath ($_prefix+"Hotfix-WindowsUpdateDatabase.txt") -Append }
            ...
            Write-Output $_line | Out-File -FilePath ($_prefix+"WindowsUpdateConfiguration.txt") -Append
            $MUSM = New-Object -ComObject "Microsoft.Update.ServiceManager"
            $MUSM.Services | Select-Object * -ExcludeProperty RedirectUrls | Out-File -FilePath ($_prefix+"WindowsUpdateConfiguration.txt") -Append 2>> $_errorfile
        }
...
function Get-DNDDoLogs
{
    ...
    if ($null -ne (Get-Service -Name dosvc -ErrorAction SilentlyContinue)) {
        ...
        FwCreateFolder $_tempdir\DOSVC
        LogInfo ("[$LogPrefixDO] Getting DeliveryOptimization logs.")
        $CommandsDOSVC = @(
            "robocopy `"$Env:windir\ServiceProfiles\NetworkService\AppData\Local\Microsoft\Windows\DeliveryOptimization\Logs`" `"$_tempdir\dosvc`" *.log *.etl /W:1 /R:1 /NP /LOG+:$_robocopy_log /S | Out-Null"
            "robocopy `"$Env:windir\SoftwareDistribution\DeliveryOptimization\SavedLogs`" `"$_tempdir\dosvc`" *.log *.etl /W:1 /R:1 /NP /LOG+:$_robocopy_log /S | Out-Null"
        )
        ...
        FwExportRegToOneFile $LogPrefixDO $RegKeysDOSVC "$_tempdir\dosvc\registry_DeliveryOptimization.txt"

        LogInfo ("[$LogPrefixDO] Getting DeliveryOptimization perf data.")
		$outfile = "$_tempdir\dosvc\DeliveryOptimization_info.txt"
		$Commands = @(
...
function Get-DNDSetupLogs
{
    ...
    if (Test-Path "$Env:UserProfile\Local Settings\Application Data\Microsoft\WebSetup\Panther") {
        ...
        $SourceDestinationPaths = New-Object 'System.Collections.Generic.List[Object]'
        $SourceDestinationPaths.add(@("$Env:LocalAppdata\microsoft\Microsoft\Windows\PurchaseWindowsLicense\PurchaseWindowsLicense.log", "$_tempdir\UpgradeSetup\PurchaseWindowsLicense"))
		FwCopyFiles $SourceDestinationPaths -ShowMessage:$False
    }
    ...
    if (Test-Path "$Env:UserProfile\Local Settings\Application Data\Microsoft\WebSetup\Panther") {
        ...
        $SourceDestinationPaths = New-Object 'System.Collections.Generic.List[Object]'
        $SourceDestinationPaths.add(@("$Env:LocalAppdata\microsoft\Microsoft\Windows\Windows Anytime Upgrade\upgrade.log", "$_tempdir\UpgradeSetup\WindowsAnytimeUpgrade"))
		FwCopyFiles $SourceDestinationPaths -ShowMessage:$False
    }
    EndFunc $MyInvocation.MyCommand.Name
}
...
function Get-DNDStorageInfo
{
    ...
    if ($_WIN8_OR_LATER -eq 1)
    {
        ...
        } catch { LogException ("[$LogPrefixStorage] Failed to get capacity and free space.") }
        ...
        } catch { LogException ("[$LogPrefixStorage] Failed to retrieve storage info from Win32_DiskDrive class.") }
...
function Get-DNDBitlockerInfo
{
    ...
    if (Test-Path "$Env:ProgramFiles\Microsoft\MDOP MBAM\mbamagent.exe") {
        try
        {
            LogInfo ("[$LogPrefixStorage] Retrieve info from MBAM_Volume class.")
            Get-CimInstance -class mbam_volume -namespace root\microsoft\mbam | Out-File -Append ($_prefix+"Bitlocker_MBAM-WMINamespace.txt")
        } catch { LogException ("[$LogPrefixStorage] Failed to retrieve info from MBAM_Volume class.") }
    }
...
function Get-DNDActivationState
{
    ...
    } catch { LogException ("[$LogPrefixActivation] Failed to retrieve domain from Win32_ComputerSystem.") }
    ...
    if (Test-Path $sessionsXMLFile)
    {
        ...
        if ($sessionsXMLSize -lt 150)
        {
            try
            {
                ...
                if (0 -ne $sessionobj.Count) { $sessionobj | Sort-Object Date | Format-Table -Property * -AutoSize | Out-File "$_tempdir\logs\cbs\CBS_sessions_xml_sum.txt" }
                elseif (0 -eq $sessionobj.Count) { Write-Output "No problematic session found." | Out-File "$_tempdir\logs\cbs\CBS_sessions_xml_sum.txt" }
            }
            ...
            Write-Output "sessions.xml is too big ($sessionsXMLSize MB), it would need too much time to process - skipping." | Out-File "$_tempdir\logs\cbs\CBS_sessions_xml_sum.txt" 
        }
    }
#>
    EndFunc $MyInvocation.MyCommand.Name
...
function Get-DNDRFLCheckPrereqs
{
	...
	if (!$global:IsLiteMode) {
        ...
        catch { LogException "An Exception happend when running $global:PstatPath" $_ }
	...
	if (Test-Path -Path "$($_prefix)Hotfix-WMIC.txt")
    {
        ...
        $SourceDestinationPaths = New-Object 'System.Collections.Generic.List[Object]'
	    $SourceDestinationPaths.add(@("$($_prefix)Hotfix-WMIC.txt", "$($_prefix)Hotfixes.csv"))
	    FwCopyFiles $SourceDestinationPaths -ShowMessage:$False     
    }
    else
    {
        ...
        } catch { LogException ("[$LogPrefixRFL] Failed to retrieve installed updates from Win32_QuickFixEngineering class.") }
    }
    EndFunc $MyInvocation.MyCommand.Name
}
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
Function CollectDND_SETUPLog
{
    ...
    } catch { Write-Output "Failed to collect driver information from Win32_PnPEntity class.`n" }
    ...
    FwGetSysInfo -Subfolder "DnDLog$LogSuffix\System"
    try
    {
        Get-CimInstance -ClassName win32_quickfixengineering | Out-File -Append ($_EXPORTDIR + '\System\wmic_qfe_list.txt')
    } catch { Write-Output "Failed to collect driver information from Win32_QuickFixEngineering class.`n" }
    ...
    } catch { Write-Output "Failed to collect driver information from Win32_PnPSignedDriver class.`n" }
    ...
    catch { Write-Output "Failed to collect disk information from Win32_DiskDrive, Win32_DiskPartition, Win32_LogicalDiskToPartition, Win32_LogicalDisk or Win32_Volume class.`n" }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCA9iopeKuMHAiw
# LQSoiFajVCxcZUPkpEEUne0LhegkFqCCDYEwggX/MIID56ADAgECAhMzAAACzI61
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIg