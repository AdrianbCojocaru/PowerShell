...
REM Modify date 4/22/2021
REM Version 1.25
REM Author: Leonard.Severt@microsoft.com


REM - Add PS .NET Script for version info -  Added to reg_.NET-Setup.txt
REM - Add font info to App Compat  Font_Info.txt
REM - Get "C:\windows\system32\winevt\Logs\Microsoft-Client-Licensing-Platform%4Admin.evtx" in text - Done
REM - Get SOFTWARE\Microsoft\Windows\CurrentVersion\Reliability - Done
REM - Change get-features to table format - Done
REM - Get HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Power  - Done Added to existing reg_power.txt
REM - Change get-capabilites to table format - Done
REM - Get driver signing info using driverquery - Done driver_signing.txt
REM - Display warning to user if 2 report VBS files are not in folder with setupreport
...
REM ---Check for the existance of summaryreport and eventlog scripts
If NOT EXIST "%_Batchdir%\SummaryRep.txt" set _supportfiles=0
If NOT EXIST "%_Batchdir%\GetEvents.txt" set _supportfiles=0
If %_supportfiles% NEQ 0 goto FilesComplete
Echo.
Echo The report normally consist of the setupreport.cmd file, getevents.txt file and summaryrep.txt file.
Echo Either the summaryrep.txt file and/or the getevents.txt file are not in the same folder as the setupreport.cmd.
Echo Do you wish to stop the batch and copy the missing files to get a more complete report? (ignore if other files not provided)
Echo.
choice /c yn /m "Yes, No"
if %ERRORLEVEL% == 1 (
Echo Please rerun the batch after copying the files
pause
goto Good_Bye
)
...
Echo SetupReport version 1.25 > "%_Errorfile%"
...
Echo SetupReport Version is 1.25 > %_PREFIX%MiscInfo.txt
...
dir /a /s /r "C:\windows\fonts" > %_TEMPDIR%\Appcompat\dir_Fonts.txt 2>> %_Errorfile%
...
Rem - Powershell for font info
set _FONTINFO_SCRIPT=%TEMP%\FONTINFO.ps1
Echo [System.Reflection.Assembly]::LoadWithPartialName("System.Drawing") >> %_FONTINFO_SCRIPT%
Echo (New-Object System.Drawing.Text.InstalledFontCollection).Families >> %_FONTINFO_SCRIPT%
powershell -ExecutionPolicy Bypass -Command %_FONTINFO_SCRIPT% > %_TEMPDIR%\Appcompat\FontInfo1.txt 2>> %_Errorfile%
del %_VERSION_SCRIPT% 2>> %_Errorfile%

powershell -ExecutionPolicy Bypass -Command Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts' > %_TEMPDIR%\Appcompat\FontInfo2.txt 2>> %_Errorfile%
...
Dism /english /online /Get-Features /Format:Table > "%_PREFIX%Dism_GetFeatures.txt" 2>> %_Errorfile%
...
Dism /english /online /Get-Capabilities /Format:Table > "%_PREFIX%Dism_GetCapabilities.txt" 2>> %_Errorfile%
...
cscript.exe //e:vbs "%_Batchdir%\GetEvents.txt" "Microsoft-Client-Licensing-Platform/Admin" /channel /TXT /noextended >NUL 2>> %_Errorfile%
cscript.exe //e:vbs "%_Batchdir%\GetEvents.txt" "Microsoft-Client-Licensing-Platform/Admin" /channel /TXT /CSV /noextended >NUL 2>> %_Errorfile%
...
Echo %_line% >> %_PREFIX%reg_Power.txt 2>> %_Errorfile%
reg.exe query "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Power" /s >> %_PREFIX%reg_Power.txt 2>> %_Errorfile%
reg.exe query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Reliability" /s > %_PREFIX%reg_Relibility.txt 2>> %_Errorfile%
...
If %_Win10% == 1 driverquery /si > %_PREFIX%Driver_signing.txt 2>> %_Errorfile%
...
REM - Get .Net info
set _DOTNETINFO_SCRIPT=%TEMP%\dotnetinfo.ps1
Echo $DotNetVersions = Get-ChildItem HKLM:\SOFTWARE\WOW6432Node\Microsoft\Updates ^| Where-Object {$_.name -like "*.NET Framework*"} > %_DOTNETINFO_SCRIPT%
Echo ForEach($Version in $DotNetVersions){ >> %_DOTNETINFO_SCRIPT%
Echo    $Updates = Get-ChildItem $Version.PSPath >> %_DOTNETINFO_SCRIPT%
Echo     $Version.PSChildName >> %_DOTNETINFO_SCRIPT%
Echo     ForEach ($Update in $Updates){ >> %_DOTNETINFO_SCRIPT%
Echo        $Update.PSChildName >> %_DOTNETINFO_SCRIPT%
Echo        } >> %_DOTNETINFO_SCRIPT%
Echo } >> %_DOTNETINFO_SCRIPT%

Echo %_line% >> "%_PREFIX%reg_.NET-Setup.txt"
Echo Get .Net info using PS script >> "%_PREFIX%reg_.NET-Setup.txt"
Echo %_line% >> "%_PREFIX%reg_.NET-Setup.txt"

powershell -ExecutionPolicy Bypass -Command %_DOTNETINFO_SCRIPT% >> "%_PREFIX%reg_.NET-Setup.txt" 2>> %_Errorfile%
del %_DOTNETINFO_SCRIPT% 2>> %_Errorfile%
