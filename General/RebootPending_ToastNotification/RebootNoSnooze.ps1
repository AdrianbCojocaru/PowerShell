<#
.SYNOPSIS
    Create a toast notification asking the logged on user to reboot their device.

.DESCRIPTION
    Settings are customizable through configuration xml file.
    All actions are logged to a local log file in AppData\Roaming\ToastNotificationScript\New-ToastNotification.log.
    Meant to run from a scheduled task every one hour if the device uptime is greater than MaxUptime days giving users the option to reboot.
    If not reboot happens then the reboot is forced after the deadline expires.

.PARAMETER Config
    Specify the path for the config.xml. If none is specified, the script uses the local config.xml

.NOTES
    Based on the toast notifications cript from Martin Bengtsson
        ...
        ToastRunUpdateID {
            try {
                ...
                try {
                    ...
                    [String]$Script = "powershell.exe -ExecutionPolicy Bypass -NoLogo -NonServiceDeskive -NoProfile -WindowStyle Hidden -File `"$global:CustomScriptsPath\ToastRunUpdateID.ps1`""
...
'@
        ...
        ToastReboot {
            try {
                ...
                try {
                    ...
                    $MyCompanyDeviceUptimeTS = (Get-ScheduledTask MyCompanyDeviceUptime* |  Get-ScheduledTaskInfo)
                    [String]$script ='@echo off'
                    if ($MyCompanyDeviceUptimeTS) {
                        $TSName = $MyCompanyDeviceUptimeTS[0].TaskName
                        Write-Log "MyCompanyDeviceUptime Scheduled Task found: $TSName"
                        [String]$script +="`r`nschtasks /delete /tn $TSName /F"
                    } else {
                        Write-Log "No MyCompanyDeviceUptime Scheduled Task found"
                    }
                    $Script += "`r`nshutdown /r /t 0 /d p:2:2222 /c `"Manual Toast Notification Reboot Button`""
                    $MyCompanyDeviceRebootTS = (Get-ScheduledTask MyCompanyDeviceReboot* |  Get-ScheduledTaskInfo)
                    if ($MyCompanyDeviceRebootTS) {
                        $TSName = $MyCompanyDeviceRebootTS[0].TaskName
                        Write-Log "MyCompanyDeviceReboot Scheduled Task found: $TSName"
                        ## This TS is needed as shutdown.exe or Restart-Computer do not work if the user is not a local admin
                        $Script += "`r`nschtasks /run /tn $TSName"
                    } else {
                        Write-Log "No MyCompanyDeviceReboot Scheduled Task found"
                    }
                    if (-NOT[string]::IsNullOrEmpty($Script)) {
                        Out-File -FilePath $GetCustomScriptPath -InputObject $Script -Encoding ASCII -Force
                    }
        ...
        ToastRunPackageID {
            try {
                ...
                try {
                    $GetCustomScriptPath = $PathInfo.FullName
                    [String]$Script = "powershell.exe -ExecutionPolicy Bypass -NoLogo -NonServiceDeskive -NoProfile -WindowStyle Hidden -File `"$global:CustomScriptsPath\ToastRunPackageID.ps1`""
                    if (-NOT[string]::IsNullOrEmpty($Script)) {
                        Out-File -FilePath $GetCustomScriptPath -InputObject $Script -Encoding ASCII -Force
                    }
                }
...
'@
        ...
        ToastRunApplicationID {
            try {
                ...
                try {
                    $GetCustomScriptPath = $PathInfo.FullName
                    [String]$Script = "powershell.exe -ExecutionPolicy Bypass -NoLogo -NonServiceDeskive -NoProfile -WindowStyle Hidden -File `"$global:CustomScriptsPath\ToastRunApplicationID.ps1`""
                    if (-NOT[string]::IsNullOrEmpty($Script)) {
                        Out-File -FilePath $GetCustomScriptPath -InputObject $Script -Encoding ASCII -Force
                    }
                }
...
$global:RegistryPathDetection = "HKCU:\SOFTWARE\ToastNotificationScript\Detection"
...
[bool]$MyCompanyLogoRebootSameHash = $false
[string]$MyCompanyLogoRebootHash = '914FD40329EEBC9F8651180BF0F01780251F80790CA6525FB04F36B51BFBD0'
[string]$MyCompanyLogoRebootUrl = 'https://uswest.blob.core.windows.net/comressources/MyCompanyLogo.png?5SBZSA%3D'
$MyCompanyLogoPath = "$env:temp\MyCompanyLogoReboot.png"
[bool]$ServiceDeskRebootSameHash = $false
[string]$ServiceDeskRebootHash = 'E268F598218B463248938409ADA638206C902172D29C3C987FD750C69'
[string]$ServiceDeskRebootUrl = 'https://weuasadwcmemprd01.blob.core.windows.net/comressources/ServiceDesk.png?XSDs5SBZSA%3D'
$ServiceDeskPath = "$env:temp\ServiceDesk.png"
#EndRegion ===============================[Variables]=============================
#endregion

#region Main Process
####
if (Test-Path $MyCompanyLogoPath) {
    if ((Get-FileHash $MyCompanyLogoPath -Algorithm SHA256).Hash -eq $MyCompanyLogoRebootHash) { $MyCompanyLogoRebootSameHash = $true }
}
# If no hash match download the file and calculate its hash again
if (-not $MyCompanyLogoRebootSameHash) {
    Start-BitsTransfer $MyCompanyLogoRebootUrl -Destination $MyCompanyLogoPath
    if ((Get-FileHash $MyCompanyLogoPath -Algorithm SHA256).Hash -eq $MyCompanyLogoRebootHash) { $MyCompanyLogoRebootSameHash = $true }
}
if (-not $MyCompanyLogoRebootSameHash) { throw "MyCompany logo hashes don't match after download" }

if (Test-Path $ServiceDeskPath) {
    if ((Get-FileHash $ServiceDeskPath -Algorithm SHA256).Hash -eq $ServiceDeskRebootHash) { $ServiceDeskRebootSameHash = $true }
}
# If no hash match download the file and calculate its hash again
if (-not $ServiceDeskRebootSameHash) {
    Start-BitsTransfer $ServiceDeskRebootUrl -Destination $ServiceDeskPath
    if ((Get-FileHash $ServiceDeskPath -Algorithm SHA256).Hash -eq $ServiceDeskRebootHash) { $ServiceDeskRebootSameHash = $true }
}

if (-not $ServiceDeskRebootSameHash) { throw "ServiceDesk logo hashes don't match after download" }
####
...
if (-NOT($Config)) {
    ...
    $Config = Join-Path ($global:ScriptPath) "RebootNoSnooze.xml"
}
...
if (-NOT[string]::IsNullOrEmpty($Xml)) {
    try {
        ...
        ###     $LogoImage = $MyCompanyLogoPath
        ###     $HeroImage =$ServiceDeskPath 
        ### }
        $LogoImage = $MyCompanyLogoPath
        $HeroImage = $ServiceDeskPath
...
<toast scenario="$Scenario">
    ...
    <binding template="ToastGeneric">
        ...
        <group>
            <subgroup>     
                <text hint-style="body" hint-wrap="true">Please reach out to ServiceDesk HelpDesk, should you require additional support.</text>
            </subgroup>
        </group>
    </binding>
    </visual>
...
$DeadlineDate = Get-Date $DeadlineContent
if (($ActionButton1Enabled -eq "True") -AND ($DismissButtonEnabled -ne "True")) {
    Write-Log -Message "Creating the xml for no dismiss button"
    try {
        [datetime]$DetectionDate = (Get-ItemProperty -Path $RegistryPathDetection -Name 'DetectionDate').DetectionDate
    }
    catch {
        [datetime]$DetectionDate = '11/5/1980 8:27:29 PM'
    }
    if ($CurrentDate -gt $DeadlineDate) {
        $TitleText = "Your device will be automatically restarted in 10 minutes!"
        $BodyText1 = "As your restart deadline has passed, your device will restart automatically in 10 minutes. Make sure you save all your open files!"
        $BodyText2 = "Restart manually by clicking now on the `"Restart now`" button below."
        Write-Log -Message "Deadline [$DeadlineContent] passed. 10 minutes message displayed."
    }
    [xml]$Toast = @"
<toast scenario="$Scenario">
    <visual>
...
if (($DeadlineEnabled -eq "True") -OR ($DynDeadlineEnabled -eq "True")) {
    ...
    if ($DeadlineContent) {
        ...
        if ($CurrentDate -gt $DeadlineDate) {$DeadlineText = "Your deadline was:"}
        $DeadlineGroup = @"
        <group>
            <subgroup>
...
#Region -------------------------------------------------------- [MyCompany Code] ----------------------------------------------

$Uptime = Get-DeviceUptime

$DeadlineDateStr = $DeadlineDate.ToString()
Write-Log -Message "DeadlineDate = $DeadlineDateStr"
if ($Uptime -le $MaxUptimeDays ) {
    Write-Log -Message "Current uptime: $Uptime days, less the maximum defiled uptime: $MaxUptimeDays days"
    #delete reg
    # delete schheduled task
}
else {
    Write-Log -Message "Current uptime: $Uptime days, greater than or equal to the maximum defiled uptime: $MaxUptimeDays days"
    if ($CurrentDate -ge $DeadlineDate) {
        #if (Test-Path $RegistryPathDetection) {
        #    [datetime]$DetectionDate = (Get-ItemProperty -Path $RegistryPathDetection -Name 'DetectionDate').DetectionDate
        #    if ($DetectionDate.AddDays(-1) -ge $CurrentDate) {
        # if the app ran already on this system this reboot itteration
        #Display-ToastNotification
        $CancelPendingReboot = Start-Process -WindowStyle Hidden -FilePath shutdown.exe -ArgumentList '/a' -PassThru -Wait
        Write-Log -Message "Current reboot pending canceled exit code: $($CancelPendingReboot.ExitCode). Legend: 0 = Ok; 1116 = Nothing pending"
        $Restart12Minutes = Start-Process -WindowStyle Hidden -FilePath shutdown.exe -ArgumentList '/r /t 600 /d p:3:3333' -PassThru -Wait
        Display-ToastNotification
        $RebootDate = $CurrentDate.AddSeconds(600).ToString()
        Write-Log -Message "Reboot scheduled for: $RebootDate. Return code: $($Restart12Minutes.ExitCode)"
        #   } else {
        #        Display-ToastNotification
        #   }###
        #}
        ...
        if ($CurrentDate.AddHours(1) -gt $DeadlineDate) {
            Write-Log -Message "3h or less deadline; deadline date $DeadlineDateStr is in 3h or less."
            [int32]$SecondsLeft = $DeadlineDate.Subtract($CurrentDate).TotalSeconds
            $CancelPendingReboot = Start-Process -WindowStyle Hidden -FilePath shutdown.exe -ArgumentList '/a' -PassThru -Wait
            Write-Log -Message "3h or less deadline; current reboot pending canceled exit code: $($CancelPendingReboot.ExitCode). Legend: 0 = Ok; 1116 = Nothing pending"
            Write-Log -Message "Seconds left until reboot: $SecondsLeft"
            $Restart12Minutes = Start-Process -WindowStyle Hidden -FilePath shutdown.exe -ArgumentList "/r /t $SecondsLeft /d p:3:3333" -PassThru -Wait
            Write-Log -Message "3h or less deadline; reboot scheduled for: $RebootDate. Return code: $($Restart12Minutes.ExitCode)"
            Display-ToastNotification
        }
        else { Display-ToastNotification }
    }
}

#EndRegion ----------------------------------------------------- [MyCompany Code] ----------------------------------------------
