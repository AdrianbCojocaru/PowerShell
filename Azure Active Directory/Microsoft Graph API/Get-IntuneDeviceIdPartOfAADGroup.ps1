...
[string]$tenantId = if ($env:AZUREPS_HOST_ENVIRONMENT) { Get-AutomationVariable -Name "memTenantId" } else { $env:MyTenantId }
[string]$clientId = if ($env:AZUREPS_HOST_ENVIRONMENT) { Get-AutomationVariable -Name "memEmailAppClientId" } else { $env:MyDefenderAppClientId }
[string]$appSecret = if ($env:AZUREPS_HOST_ENVIRONMENT) { Get-AutomationVariable -Name "memEmailAppSecret" } else { $env:MyDefenderAppSecret }
